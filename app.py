...
with header_col2:
    st.title('Optimizing Crop Rotation for Small-scale Farmers')
...
with col1:
    ...
    if st.button("Predict Top Crops", key='predict_crops'):
        ...
        st.header("Crop History & Current Plan")
    past_crops = st.multiselect("Select past season crops:", crop_images.keys())
    current_crop_number = st.number_input("Enter number of crops grown this year:", min_value=1, max_value=5, value=1)
